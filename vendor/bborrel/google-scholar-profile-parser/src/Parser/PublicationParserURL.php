 ...
 */
class PublicationParserURL extends BaseParser implements Parser
{
    ...
    public const GSCHOLAR_CSS_CLASS_REFERENCE_TITLE = 'gsc_vcd_title';
    public const GSCHOLAR_CSS_CLASS_CITATION = 'gsc_a_c';
    public const GSCHOLAR_CSS_CLASS_YEAR = 'gsc_a_y';
    public const GSCHOLAR_CSS_CLASS_GRAY = 'gsc_vcd_value';
    ...
    public function parse() : array
    {
        ...
        $crawlerPublications = $this->crawler;
    ...
    {
        $title = $publicationPath = $authors = $year = $publisherDetails = '';
        $childNodeIndex = 0;

        /** @var DOMElement $childNode */
        foreach ($node->childNodes as $childNode) {
            $cssId = $childNode->getAttribute('id');
            $cssClass = $childNode->getAttribute('class');

            if ($cssId === self::GSCHOLAR_CSS_CLASS_REFERENCE_TITLE) {
                $title = $childNode->textContent;
                //$publicationPath = $childNode->getAttribute('data-href');
            }

            if ($cssClass === self::GSCHOLAR_CSS_CLASS_GRAY && $childNodeIndex === 1) {
                $authors = $childNode->textContent;
            } elseif ($cssClass === self::GSCHOLAR_CSS_CLASS_GRAY && $childNodeIndex === 2) {
                  $year = $childNode->textContent;
            } elseif ($cssClass === self::GSCHOLAR_CSS_CLASS_GRAY && $childNodeIndex === 3) {
                $publisherDetails = $childNode->textContent;
            }

            ++$childNodeIndex;
        }

        return compact('title', 'publicationPath', 'authors', 'year', 'publisherDetails');
    }
